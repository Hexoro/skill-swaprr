...
-- ═══════════════════════════════════════════════════════
-- Migration 2: Forums + Blog tables
-- ═══════════════════════════════════════════════════════
...
CREATE POLICY "Users can delete their own threads" ON public.forum_threads FOR DELETE USING (auth.uid() = author_id);
...
CREATE POLICY "Forum votes are viewable by everyone" ON public.forum_votes FOR SELECT USING (true);
...
-- Blog RLS
CREATE POLICY "Published blog posts viewable by everyone" ON public.blog_posts FOR SELECT USING (is_published = true OR auth.uid() = author_id);
CREATE POLICY "Authenticated users can create blog posts" ON public.blog_posts FOR INSERT WITH CHECK (auth.uid() = author_id);
...
CREATE POLICY "Authenticated users can create blog comments" ON public.blog_comments FOR INSERT WITH CHECK (auth.uid() = author_id);
CREATE POLICY "Users can delete their own blog comments" ON public.blog_comments FOR DELETE USING (auth.uid() = author_id);
CREATE POLICY "Blog likes viewable by everyone" ON public.blog_likes FOR SELECT USING (true);
...
INSERT INTO public.forum_categories (name, slug, description, icon, color) VALUES
  ('General Discussion', 'general-discussion', 'Chat about anything skill-related', 'MessageSquare', 'blue'),
  ('Skill Showcase', 'skill-showcase', 'Show off your work and get feedback', 'Palette', 'purple'),
  ('Looking for Skills', 'looking-for-skills', 'Find people with the skills you need', 'Search', 'green'),
  ('Tips & Tutorials', 'tips-tutorials', 'Share your knowledge and learn from others', 'BookOpen', 'orange'),
  ('Bug Reports', 'bug-reports', 'Report platform issues', 'Bug', 'red'),
  ('Feature Requests', 'feature-requests', 'Suggest new features', 'Lightbulb', 'yellow')
